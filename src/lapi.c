...
LUA_API void lua_settop(lua_State* L, int idx) {
    CallInfo* ci = L->ci;
    StkId func = ci->func;
    ptrdiff_t diff = 0;  /* difference for new top */
    lua_lock(L);
    if (idx >= 0) {
...
LUA_API size_t lua_stringtonumber(lua_State* L, const char* s) {
    size_t sz = luaO_str2num(L, s, s2v(L->top));
    if (sz != 0)
        api_incr_top(L);
    return sz;
}
...
LUA_API void lua_pushboolean(lua_State* L, int b) {
    ...
    /*RC:YILIN*/
    if (b) {
        setbtvalue_subref(L, s2v(L->top));
    }
    else {
        setbfvalue_subref(L, s2v(L->top));
    }
    api_incr_top(L);
    lua_unlock(L);
}
...
LUA_API void lua_pushlightuserdata(lua_State* L, void* p) {
    lua_lock(L);
    /*RC:YILIN*/
    setpvalue_subref(L, s2v(L->top), p);
    api_incr_top(L);
    lua_unlock(L);
}
...
LUA_API int lua_pushthread(lua_State* L) {
    lua_lock(L);
    /*RC:YILIN*/
    setthvalue_subref(L, s2v(L->top), L);
    api_incr_top(L);
    lua_unlock(L);
...
LUA_API void lua_settable(lua_State* L, int idx) {
    ...
    /*RC:YILIN*/
    setnilvalue_subref(L, s2v(L->top - 2));
...
static void aux_rawset(lua_State* L, int idx, TValue* key, int n) {
    ...
    /*RC:YILIN*/
    /*
     setnilvalue_subref(L, L->top - 2);
     setnilvalue_subref(L, L->top - 1);
     */
    L->top -= n;
    lua_unlock(L);
}
...
LUA_API int lua_setmetatable(lua_State* L, int objindex) {
    ...
    default: {
        if (G(L)->mt[ttype(obj)] != 0)
            luaRC_subref_object(L, (GCObject*)G(L)->mt[ttype(obj)]);
...
LUA_API const char* lua_setupvalue(lua_State* L, int funcindex, int n) {
    ...
    if (name) {
        /*RC:YILIN*/
        setobj_subref(L, val, s2v(L->top - 1));
        L->top--;
